        ...
        while (tmppacket.size > 0)
        {
            ...
            for (unsigned int i = 0; i < subtitle.num_rects; i++)
            {
                ...
                case SUBTITLE_ASS:
                    ...
                    if (rect->text)
                    {
                        // Use text approximation if available
                        box.str += rect->text;
                        box.str += '\n';
                    }
                    else
                    {
                        ...
                        msg::wrn(_url + ": subtitle stream " + str::from(_subtitle_stream)
                                + ": guessing text approximation for ASS subtitle");
                        const char *text_pos = rect->ass;
                        for (int j = 0; j < 3; j++)
                        {
                            if ((text_pos = strchr(text_pos, ',')))
                            {
                                text_pos++;
                            }
                            else
                        ...
                        box.str += text_pos ? text_pos : "";
                        if (box.str.length() > 0 && box.str[box.str.length() - 1] != '\n')
                        {
                            box.str += '\n';
                        }
                    }
